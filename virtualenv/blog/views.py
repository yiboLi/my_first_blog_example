...
def add_comment_to_post(request, pk): #pass in post's pk, not comment's
    post = get_object_or_404(Post, pk=pk)
    if request.method == "POST":
        form = CommentForm(request.POST) #after submit the comment, brought back to the same view with all data in request.POST
        if form.is_valid():
...
@login_required
def comment_approve(request, pk): #pass in comment's pk
    comment = get_object_or_404(Comment, pk=pk)
    comment.approve()
    return redirect('blog.views.post_detail', pk=comment.post.pk)
    
@login_required
def comment_remove(request, pk): #pass in comment's pk
    comment = get_object_or_404(Comment, pk=pk)
